...
export const getImportFields = (shape, formatMessage) => {
  ...
  const values = reduce(fields, (memo, field) => {
    const value = `${field.required ? formatMessage(appMessages.import.required) : ''}${formatMessage(appMessages.importFields[field.attribute])} | ${formatMessage(appMessages.import[field.type], { format: DB_DATE_FORMAT })}`;
