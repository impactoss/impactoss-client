...
export const selectTaxonomies = createSelector(
  ...
  (id, taxonomies, categories, associations) => prepareTaxonomiesIsAssociated(taxonomies, categories, associations, 'tags_measures', 'measure_id', id)
);
...
export const selectRecommendationsAssociated = createSelector(
  ...
  (id, entities, associations) => entitiesIsAssociated(entities, 'recommendation_id', associations, 'measure_id', id)
);
...
export const selectRecommendations = createSelector(
  ...
  (state) => selectEntities(state, 'categories'),
  (recommendations, connections, recMeasures, recCategories, recIndicators, categories) => recommendations && recommendations
    .map((rec) => rec
...
export const selectIndicatorsAssociated = createSelector(
  ...
  (id, entities, associations) => entitiesIsAssociated(entities, 'indicator_id', associations, 'measure_id', id)
);
// selectIndicators,
export const selectIndicators = createSelector(
  ...
  (state) => selectEntities(state, 'measure_indicators'),
  (state) => selectEntities(state, 'recommendation_indicators'),
  (indicators, connections, indicatorMeasures, indicatorRecs) => indicators && indicators
    .map((indicator) => indicator
      .set('measures', indicatorMeasures && indicatorMeasures
        .filter((association) => attributesEqual(association.getIn(['attributes', 'indicator_id']), indicator.get('id'))
          && connections.getIn(['measures', association.getIn(['attributes', 'measure_id']).toString()]))
        .map((association) => association.getIn(['attributes', 'measure_id'])))
